  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-14 hover:w-56 bg-[#0f1018] border-r border-[#1c1e2e] flex flex-col justify-between transition-all duration-300 z-50 overflow-hidden group">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center w-full px-4 py-3 gap-4 whitespace-nowrap outline-none ${active ? 'border-l-2 border-[#7c3aed] bg-[#1a103a]' : 'border-l-2 border-transparent hover:bg-[#141622]'}`}
            >
      ...
      <div className="flex flex-col py-4 border-t border-[#1c1e2e]">
        <Link
          href="/settings"
          className={`flex items-center w-full px-4 py-3 gap-4 whitespace-nowrap outline-none ${location === '/settings' ? 'border-l-2 border-[#7c3aed] bg-[#1a103a]' : 'border-l-2 border-transparent hover:bg-[#141622]'}`}
        >
